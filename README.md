...
- **Instagram posting for the 6 starter posts still pending** — the token Andrew gave as "IG access token" turned out to be a Facebook Page token (`EAA...` prefix, authenticated as Page id `1343922692139344`/"Faraway Folio HQ" via `graph.facebook.com`), not the Instagram Login API token (`IGAA...` prefix, `graph.instagram.com`) — used it for Facebook instead. Still needs a real `IG_ACCESS_TOKEN` plus a `git push` (Instagram requires the image at a live public URL) to post these to Instagram.
