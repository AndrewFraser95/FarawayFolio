...
**These live in a local `.env` file at the repo root** (gitignored, never committed) so Andrew doesn't have to repaste them every session — every pipeline script auto-loads it via `content-pipeline/env_loader.py` (stdlib only, reads `.env` into `os.environ` without overriding real env vars if already set). To update a token, just edit `.env` directly. Note: Instagram tokens (`IGAA...` prefix, from Instagram Login API) and Facebook Page tokens (`EAA...` prefix, from the Facebook Graph API) look similar but are not interchangeable — verify a new token read-only (`GET graph.instagram.com/me` or `GET graph.facebook.com/me`) before assuming which one it is.
...
- ~~European-city engagement images generated, not yet posted~~ — all 6 (`content-pipeline/starter_content.json`: Rome, Paris, Barcelona, Amsterdam, Santorini, Lisbon) generated, committed locally to `docs/media/2026-09-15/`, and **posted live to Facebook** 2026-09-15 (Page "Faraway Folio HQ").
- **Instagram posting for the 6 starter posts still pending a push** — real `IG_ACCESS_TOKEN` confirmed working (verified read-only against `graph.instagram.com/me`: id `28615169324810269`, username `farawayfolio`). Still needs a `git push` first, since Instagram requires the image to be live at a public URL (`https://farawayfolio.com/docs/media/...`), unlike Facebook's direct-upload flow.
