...
- **New European-city engagement posts not yet generated/posted** — `content-pipeline/starter_content.json` + `publish_starter_content.py` are built and ready; blocked this session on missing `IG_ACCESS_TOKEN`/`FB_PAGE_ACCESS_TOKEN` env vars.
